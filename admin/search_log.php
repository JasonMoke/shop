 ...
 * ========================================================== * $Author: Jason $
 * $Id: search_log.php 17217 2011-01-19 06:29:08Z Jason $
 */
...
function get_search_log()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('keywords') . " WHERE  searchengine='SHOP' $where";
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    $logdb = array();
    $filter = page_and_size($filter);
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('keywords') .
           " WHERE  searchengine='SHOP' $where" .
